...
def get_data(xlsx_file: str):
    ...
    # data starts at row 7; 100 goes well beyond where we expect data to end so ignore when a row is all None values
    for row in ws.iter_rows(min_row=7, max_col=12, max_row=100, values_only=True):
        ...
        if row[1] and row[1].startswith('ADGC '):
            ...
            slash_type = '–' if '–' in row[1] else '-'
            current_group = row[1].split(slash_type)[1].strip()
